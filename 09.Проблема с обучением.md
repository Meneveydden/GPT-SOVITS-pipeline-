...
# ОБНОВЛЕНИЕ s2_train.py

Класс, файл вижу, спасибо 👀 

Сначала коротко отвечу на вопрос про дискриминатор:

> А ничего страшного, если в модели s2Dv2Pro.pth нет русского языка?

**Нет, не страшно.**
Дискриминатор (`s2D`) вообще не смотрит на текст/язык — он видит только аудио (волновую форму/мел-спектр) и учится отличать “реальный звук” от “сгенерированного”. Он выучил общие паттерны речи (тембр, естественность, артефакты), а не конкретный язык. Так что его спокойно можно использовать как есть.

Критичная проблема — только у генератора (`s2G`), где есть `enc_p.text_embedding.weight` с другим размером.

---

## Где именно править `s2_train.py`

Нужно поменять кусок в функции `run`, в блоке `except`, где загружается `pretrained_s2G`. Вот этот участок (он у тебя ровно так выглядит): 
...
### Заменяем этот блок на версию с “дропом” `enc_p.text_embedding.weight`

Полностью замени **весь** этот `if (hps.train.pretrained_s2G...)` на:
...
```python
        if (
            ...
            # В разных файлах ключ может называться "weight" или "model"
            state_dict_g = ckpt_g.get("weight", ckpt_g.get("model", ckpt_g))

            # Ключ проблемного слоя
            emb_key = "enc_p.text_embedding.weight"

            if emb_key in state_dict_g:
                ckpt_emb = state_dict_g[emb_key]
                # Берём текущий слой из модели (уже с твоим размером 765)
                if torch.cuda.is_available():
                    cur_emb = net_g.module.enc_p.text_embedding.weight
                else:
                    cur_emb = net_g.enc_p.text_embedding.weight

                # Если размеры не совпадают — выкидываем этот слой из чекпойнта
                if ckpt_emb.shape != cur_emb.shape:
                    print(
                        f"[s2_train] Dropping {emb_key} due to size mismatch: "
                        f"{ckpt_emb.shape} vs {cur_emb.shape}"
...
Остальной код в `except` **оставляешь как есть**, включая загрузку `pretrained_s2D` для дискриминатора.

### Небольшая доп. правка для `s2D` (по желанию, но полезно)

Чуть ниже у тебя загрузка дискриминатора:
...
Здесь для CPU-ветки `strict` по умолчанию `True`. Чтобы не поймать аналогичную ошибку в будущем, можно чуть подправить:
...
1. Вносишь эти правки в `s2_train.py`.
2. Ничего больше в конфиге `tmp_s2.json` менять не надо, `phoneme_vocab_size` оставляем как ты уже поставила.
...
В логах при старте должно появиться что-то вроде:
...
Если хочешь, после правки можешь прислать первые строки лога запуска s2 — проверим, что оно теперь подхватило pretrain как надо и ушло в обучение, а не падает на `size mismatch`.
